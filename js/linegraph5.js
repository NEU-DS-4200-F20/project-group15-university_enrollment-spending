...
function linegraph5() {
    ...
    d3.csv("data/mergedDataFinal2.csv").then(function (data) {
        ...
        for (const school of schoolsData) {
            for (const el of data) {
                if (el.SchoolName === school.name) {
                    school.points.push({ year: el.YearData, value: Number(el.AuxiliaryEnterprises_Expenses) });
                }
            }
        }
        ...
        var y = d3.scaleLinear()
            .domain([0, d3.max(data, function (d) { return +Number(d.AuxiliaryEnterprises_Expenses); })])
            .range([height, 0]);
        svg.append("g")
            .call(d3.axisLeft(y));
